...
// ── MomentumBanner ────────────────────────────────────────────────────────
function MomentumBanner({ sentiment, brentChange, reason, stale }) {
...
{stale && <span style={{ fontSize: “7.5px”, color: “#555”, fontFamily: “sans-serif”, marginLeft: “4px” }}>↑ prev</span>}
</div>
...
const [sentimentStale, setSentimentStale] = useState(false);
...
try {
  ...
  if (data.west?.length && data.iran?.length && data.rucn?.length) {
    ...
    // Need at least 3 recent non-neutral articles for a reliable score
    // If not enough, keep previous score/reason (they remain unchanged)
    setSentimentStale(now6.length < 3);
    if (now6.length >= 3) {
...
<style>{`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;600;700;900&family=Source+Sans+3:ital,wght@0,300;0,400;0,600;1,400&display=swap'); * { box-sizing: border-box; margin: 0; padding: 0; } @keyframes pulse  { 0%,100%{opacity:1} 50%{opacity:0.35} } @keyframes fadeIn { from{opacity:0;transform:translateY(5px)} to{opacity:1;transform:translateY(0)} } @keyframes spin   { from{transform:rotate(0deg)} to{transform:rotate(360deg)} } ::-webkit-scrollbar { display: none; } .headline:hover  { color: #1a3a6b !important; transition: color 0.15s; } .tab-btn         { transition: background 0.12s, color 0.12s, border-color 0.12s; } .filter-btn      { transition: background 0.12s, color 0.12s, border-color 0.12s; } .tab-btn:hover   { background: #f0ede6 !important; } .filter-btn:hover{ border-color: #999 !important; color: #333 !important; } .drawer-overlay { animation: overlayIn 0.2s ease; } .drawer-panel   { animation: drawerUp 0.28s cubic-bezier(0.32,0.72,0,1); } @keyframes overlayIn { from{opacity:0} to{opacity:1} } @keyframes drawerUp  { from{transform:translateY(100%)} to{transform:translateY(0)} } .close-btn:hover { background: #e5e5e5 !important; } .read-btn:hover  { background: #333 !important; }`}</style>
...
```
  ...
  <div style={{ position: "sticky", top: 0, zIndex: 100, background: "#fff", borderBottom: "1px solid #ddd", boxShadow: "0 1px 6px rgba(0,0,0,0.06)" }}>
    <MomentumBanner sentiment={sentiment} brentChange={brentChange} reason={sentimentReason} stale={sentimentStale} />
